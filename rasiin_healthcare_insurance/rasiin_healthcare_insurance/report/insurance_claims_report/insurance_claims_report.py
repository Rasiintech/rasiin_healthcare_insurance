...
def get_data(filters):
    ...
    # New: insurance payments from PE Reference and JE Account
    insurance_payment_subquery = """
        SELECT
            reference_journal,
            SUM(insurance_paid) AS insurance_paid
        FROM (
            -- From Payment Entry Reference (via Journal Entry)
            SELECT
          ...
          -- From Journal Entry Account (paying a Journal Entry)
                SELECT
                    si.reference_journal,
                ...
                AND si.docstatus = 1  -- Exclude canceled sales invoices
    ...
    if filters.get("detailed"):
        ...
        query = f"""
            SELECT 
                ...
                COUNT(si.name) AS total_invoices,
                SUM(si.insurance_coverage_amount) AS total_insurance_amount,
                SUM(COALESCE(payments.insurance_paid, 0)) AS total_payment_amount,
...
#                 si.payable_amount AS patient_amount,
#                 si.patient_paid AS patient_paid,
#                 COALESCE(per_summary.insurance_paid, 0) AS insurance_paid,
#                 (si.insurance_coverage_amount - COALESCE(per_summary.insurance_paid, 0)) AS insurance_outstanding,
#                 si.status AS invoice_status,
#                 p.dob AS dob,
#                 si.policy_number AS policy_number
#             FROM 
...
#                 GROUP BY 
#                     per.reference_name
#             ) per_summary ON per_summary.journal_entry = si.reference_journal
#             {where_clause}
#             ORDER BY 
#                 p.patient_name ASC
#         """
...
#                           AND pe2.docstatus = 1
#                           AND per2.reference_name = si.reference_journal
#                     ), 0)
#                 ) AS total_payment_amount,
#                 SUM(si.insurance_coverage_amount) 
#                     - SUM(
#                         COALESCE((
#                             SELECT SUM(per2.allocated_amount)
#                             FROM `tabPayment Entry Reference` per2
