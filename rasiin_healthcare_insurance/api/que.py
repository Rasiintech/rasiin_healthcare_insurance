...
def create_que_order_bill(doc, method=None):
    try:
        ...
        frappe.log_error(f"Error in create_que_order_bill: {str(e)}")
        frappe.throw(_("There was an error creating the sales order or invoice. Please try again."))
...
def create_sales_order(doc, customer, amount):
    ...
    try:
        ...
        frappe.log_error(f"Error in create_sales_order: {str(e)}")
        frappe.throw(_("There was an error creating the sales order."))
...
def make_sales_invoice_direct(source_name, payable_amount, mode_of_payment=None, doc=None):
    try:
        ...
        frappe.log_error(f"Error in make_sales_invoice_direct: {str(e)}")
        frappe.throw(_("There was an error creating the sales invoice."))
